...
class amazon_review_scraper:
	...
	def build_rating(self, review):
		print("3-" +str(review).split("<span class=\"a-icon-alt\">")[1].split("</span>")[0].split(" ")[0])
		return str(review).split("<span class=\"a-icon-alt\">")[1].split("</span>")[0].split(" ")[0]

	def build_title(self, review):
		print("4-" + str(str(review).split("data-hook=\"review-title\"")[1].split("\">")[1].split("</a>")[0].split()))
		return str(review).split("data-hook=\"review-title\"")[1].split("\">")[1].split("</a>")[0].split()

	def build_date(self, review):
		return str(review).split("data-hook=\"review-date\">")[1].split("</span>")[0].split()
	...
	def build_body(self, review):
		...
		body = body.replace("<br>", ".").replace("<br/>", ".").replace("</br>", ".").strip().split()
		return body
	...
	def write_csv(self, file_name):
		...
		with open((file_name + '.csv'), 'w', encoding = 'utf-8-sig') as csv_file :
